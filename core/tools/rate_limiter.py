...
# Strict WAF / anti-bot INTERSTITIAL fingerprints, matched on a response BODY.
# Deliberately narrow — specific vendor challenge markers, NOT generic words
# like "security"/"forbidden"/"blocked" — so a legitimate 200 page is never
# downgraded and real findings are never suppressed.
CHALLENGE_SIGNATURES = (
    ...
    "the request could not be satisfied",   # CloudFront / AWS WAF block page
    "generated by cloudfront",
    "<title>access denied</title>",
)


def is_waf_challenge(body: str) -> Tuple[bool, str]:
    """True when a response BODY is a WAF/anti-bot interstitial (not real
    content). High-precision markers only — never generic words — so a
    legitimate page is not misclassified and its findings suppressed. Returns
    (is_challenge, matched_marker)."""
    if not body:
        return False, ""
    low = body.lower()

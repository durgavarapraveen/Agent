    ...
    def get_or_create_task(self, spec: TaskSpec) -> Tuple[Task, bool]:
        ...
        if duplicate:
            ...
            if duplicate.status in (TaskStatus.COMPLETED, TaskStatus.RUNNING, TaskStatus.CREATED, TaskStatus.QUEUED, TaskStatus.WAITING_DEPENDENCY):
                ...
                logger.info(f"TASK_DEDUP_BYPASS_UNFINISHED: Previous task={duplicate.spec.task_id} has non-active status={duplicate.status.value}. Creating fresh task for capability={spec.capability.value}")

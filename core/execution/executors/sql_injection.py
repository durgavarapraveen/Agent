...
_SQL_ERROR_RE = re.compile("|".join(SQL_ERROR_SIGNATURES), re.IGNORECASE)


class SQLiExecutor(ExecutorBase):
    ...
    def validate_target(self, endpoint: Dict[str, Any], identity: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if not accepts:
            return False, "Endpoint must accept query or body parameters"
        return True, None
        ...
        try:
            ...
            for payload in params["payloads"]:
                ...
                try:
                    ...
                    error_detected = bool(_SQL_ERROR_RE.search(body))
            ...
            elapsed = (time.monotonic() - start) * 1000
            logger.error("SQLiExecutor failed: %s", exc)
